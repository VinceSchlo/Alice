...
//
    me.CalAdd = function (data) {
        ...
        if (typeof (dateEl) == 'undefined')
            data.inputFieldId = '';
        if (typeof (data.hideOnClick) != 'boolean')
            data.hideOnClick = 1;
        ...
        if (data.buttonTitle == null || data.buttonTitle.length <= 0) {
            if (dateEl != null) {
                ...
                dateEl.addEventListener('click', function () {
                    me.CalDoFromField(data.inputFieldId, 'toogle');
                }, false);
            }
        ...
        if (data.navType != '00')
            divW.innerHTML += '<p id="calendarTitle' + id + '" class="calendarTitle"></p>';
    ...
    me.CalDoFromField = function (fieldId, action) {
        for (var i = 0; i < me.jsSDPObj.length; i++) {
            if (me.jsSDPObj[i]['inputFieldId'] == fieldId) {
                if (action == 'toogle')
                    me.CalToogle(me.jsSDPObj[i]['id']);
                if (action == 'show')
                    me.CalShow(me.jsSDPObj[i]['id']);
                if (action == 'hide')
                    me.CalHide(me.jsSDPObj[i]['id']);
                break;
            }
        }
    }
    ...
    me.CalToogle = function (id) {
        ...
        if (e.style.display == 'block') {
            ...
            if (me.jsSDPObj[nb]['inputFieldId'] != '') {
                ...
                if (f != null)
                    me.CalDateInit(id, String(f.value));
            }
            me.CalShow(id);
        }
    }
...
//
    me.CalMonthNav = function (id, val) {
        ...
        if (val.charAt(0) == '+' || val.charAt(0) == '-')
            v = me.jsSDPObj[nb]['dateDisp'].getMonth() + v;
...
//
    me.CalYearNav = function (id, val) {
        ...
        var v = parseInt(val, 10);
        if (val.charAt(0) == '+' || val.charAt(0) == '-')
            v = me.jsSDPObj[nb]['dateDisp'].getFullYear() + v;
...
//
    me.CalDateInit = function (id, dateStr) {
        ...
        pos = m.indexOf('JJ');
        if (pos != -1)
            dD = parseInt(dateStr.substr(pos, 2), 10);
        var pos = m.indexOf('AAAA');
        if (pos != -1)
            dY = parseInt(dateStr.substr(pos, 4), 10);
        else {
            pos = m.indexOf('AA');
            if (pos != -1)
                dY = parseInt(dateStr.substr(pos, 2), 10) + o['dateCentury'] * 100;
        }
        pos = m.indexOf('MM');
        if (pos != -1)
            dM = parseInt(dateStr.substr(pos, 2), 10) - 1;
        today = new Date();
        if (isNaN(dD))
            dD = today.getDate();
        if (isNaN(dM))
            dM = today.getMonth();
        if (isNaN(dY))
            dY = parseInt(today.getFullYear().toString().substr(2, 2), 10) + o['dateCentury'] * 100;
...
//
    me.CalContentInit = function (nb) {
        ...
        for (j = 0; j < cal['displayNumber']; j++) {
            ...
            if (month == cal['dateSel'].getMonth() && year == cal['dateSel'].getFullYear())
                today = cal['dateSel'].getDate();
    ...
    me.CalClick = function (nb, dateStr) {
        ...
        if (cal['inputFieldId'] != '') {
            ...
            if (f != null)
                f.value = m;
            if (cal['hideOnClick'])
                document.getElementById('calendarWrap' + cal['id']).style.display = 'none';
            else
                me.CalContentInit(nb);
        ...
        // callback
        if (typeof cal['callBack'] === "function")
            cal['callBack'](dateStr);
    }
//
// affiche le titre
//
    me.CalShowTitle = function (nb) {
        if (typeof me.jsSDPObj[nb] == 'undefined')
            return 0;
